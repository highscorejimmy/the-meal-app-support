# The Meal App – Support

Welcome to the support page for **The Meal App**.

## Contact Support

If you need help, found a bug, or want to suggest a feature, please contact us:

- **Email:** highscorejimmy@icloud.com  
- **Response time:** Within 2–3 business days

## Frequently Asked Questions

### 1) The app is not loading meals
- Check your internet connection.
- Force close and reopen the app.
- Make sure you are using the latest version of the app.

### 2) A recipe looks incorrect
We do our best to keep recipe information accurate. If something looks wrong, email us with the recipe name and details.

### 3) How do I report a bug?
Please email us with:
- Your device model
- iOS version
- App version
- Steps to reproduce the issue
- Screenshot (if possible)

## Privacy Policy

Your privacy is important to us.  
Read our privacy policy here: **https://highscorejimmy.github.io/the-meal-app-support/privacy.html**

## Marketing URL

Learn more about The Meal App here: **https://highscorejimmy.github.io/the-meal-app-support/marketing.html**

## Compatibility

The Meal App supports iPhones running recent iOS versions. For best performance, keep iOS updated.

## Legal

The Meal App and its content are provided “as is” without warranties of any kind.

## Copyright

© 2026 The Meal App. All rights reserved.
